module Jobs
  class AbstractMethodCallTypeInference
    prepend BaseJob
    ...
    def to_s
      "((AbstractMethod (#{abstract_procedure.argument_names.map { "?" }.join(" ")}) ?) call#{argument_type_inferences.map { |argument_type_inference| " #{argument_type_inference.to_s}" }.join})"
    end
  end
end

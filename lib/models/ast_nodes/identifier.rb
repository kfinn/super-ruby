module AstNodes
  class Identifier
    ...
    def spawn_type_inference
      puts "searching for type_inference for #{name} within #{Workspace.current_workspace.current_super_binding.to_s}" if ENV['DEBUG']
      Workspace.current_workspace.current_super_binding.fetch_type_inference(name)
    end
    ...
    def build_bytecode!(type_inference)
      ...
      if current_super_binding.has_dynamic_binding? name
        ...
        current_workspace.current_bytecode_builder << current_workspace.current_super_binding.fetch_static_type_inference(name).value
      else
